...
public class ETchronoField {
	...
	public static void main(String[] args) {
		...
		ChronoField alignedDayOfWeekInMonth = ChronoField.ALIGNED_DAY_OF_WEEK_IN_MONTH;// 当前日期月份中的天数,重新排列周期,算是星期几;比如03/20,结果是星期六
		ChronoField alignedDayOfWeekInYear = ChronoField.ALIGNED_DAY_OF_WEEK_IN_YEAR;// 当前日期当年的天数,重新排列周期,算是星期几;比如02/19,结果是星期一;
		ChronoField alignedWeekOfMonth = ChronoField.ALIGNED_WEEK_OF_MONTH; // 当前日期月份中的天数;重新排列,能排列几个整周(向上舍入);比如03/20,结果是3个整周;
		ChronoField alignedWeekOfYear = ChronoField.ALIGNED_WEEK_OF_YEAR;// 当前日期年份中的天数;重新排列,能排列几个整周(向上舍入);比如02/19,结果是8个整周;
